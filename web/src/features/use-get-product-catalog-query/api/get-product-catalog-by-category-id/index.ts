...
export async function getProductCatalogByCategoryId(searchParams: SearchParams) {
  try {
    const res = await graphqlApi<ServerResponse>({
      data: { query: queryText, variables: searchParams },
    });
    if (res.data.errors === undefined) {
      return { data: snakeToCamelCase(res.data.data.productCatalogByCategoryId) as ProductCatalog };
    }
