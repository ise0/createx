...
export async function removeUserCartItem({ userId, productId, productCharacteristicId }: Params) {
  ...
  try {
    ...
    let queryText = `
      delete from user_cart where user_id = $1 and product_id = $2 and product_characteristic_id = $3;
    `;
